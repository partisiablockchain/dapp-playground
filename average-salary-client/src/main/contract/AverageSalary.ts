...
export function newContractState(administrator: BlockchainAddress, averageSalaryResult: Option<number>, numEmployees: Option<number>): ContractState {
...
function fromScValueContractState(structValue: ScValueStruct): ContractState {
  return {
    ...
    averageSalaryResult: structValue.getFieldValue("average_salary_result")!.optionValue().valueOrUndefined((sc1) => sc1.asNumber()),
    numEmployees: structValue.getFieldValue("num_employees")!.optionValue().valueOrUndefined((sc2) => sc2.asNumber()),
  };
}
